...
class Trimmer(ABC):
	@abstractmethod
	def trimm(self, adapters, seq):
		pass
class adapt_trimm(Trimmer):
	def trimm(self, adapters, seq):
		a_count_start = 0
		a_count_end = 0
		for char in seq:
		...
		for char in reversed(seq):
			if char == adapters:
				...
				break
		seq = seq[a_count_start:len(seq) - a_count_end]

		return seq

class poly_a_trimm(Trimmer):
	def trimm(self, adapters, seq):
		if seq[:3] == adapters and seq[-3:] == adapters[::-1]:
			# Remove the first and last 3 adapters from the target sequence if they match the input string of 3 adapters
			seq = seq[3:-3]
		return seq
...
if __name__ == '__main__':
	...
	#trimmer = adapt_trimm()
	#trimmer.trimm(adapters,target_seq)
	trimmer = poly_a_trimm()
	trimmer.trimm('A',target_seq)
